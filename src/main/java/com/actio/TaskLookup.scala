...
class TaskLookup extends Task {
  ...
  def execute(): Unit = {
    ...
    val dataSource = DPSystemFactory.newDataSource(
      dataSourceConfig.withValue("query.queryTemplate", ConfigValueFactory.fromAnyRef(query.replaceAllLiterally("$1", inClause))),
      masterConfig)

    dataSource.execute()

    val ord = dataSource.dataSet.schema.asInstanceOf[SchemaArray].content.asInstanceOf[SchemaRecord].
      fields.map(_.label).indexOf(lookupColumn2) // assuming tablular
  ...
  def lookupColumn1: String = getConfig.getString("lookup1")
  def lookupColumn2: String = getConfig.getString("lookup2")
}
