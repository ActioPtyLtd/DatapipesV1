...
trait TableScala {
  ...
  def transformFirstRowAsHeader = new DataSetTableScala(rows.head.map(_.toString), rows.tail)

  def transformWithRowFunction(columnName: String, rowFunc: List[String] => String) =  new DataSetTableScala(columnName :: header, rows.map(r => rowFunc(r) :: r))
  ...
  def transformCombineColumnsWithOtherTable(t2: TableScala, pos: Int) =  new DataSetTableScala(header.take(pos) ::: t2.header ::: header.drop(pos), (rows zip t2.rows).map(r => r._1.take(pos) ::: r._2 ::: r._1.drop(pos)))

  def transformUnion(t2: TableScala) = new DataSetTableScala(header, rows ::: t2.rows)

  def transformSelect(selectorFunc: String => Boolean) =  new DataSetTableScala(header.filter(c => selectorFunc(c)), rows.map(r => getOrdinalsWithPredicate(selectorFunc).map(c => r(c))))

  def transformSelect(columnOrdinals: List[Int]) =  new DataSetTableScala(columnOrdinals.map(header(_)), rows.map(r => columnOrdinals.map(c => r(c))))

  def transformFilter(filter: List[String] => Boolean) =  new DataSetTableScala(header, rows.filter(filter))

  def transformRename(selectorFunc: String => Boolean, renameFunc: String => String) =  new DataSetTableScala(header.map(c => if(selectorFunc(c)) renameFunc(c) else c), rows)
...
//  }
  ...
  def union(tables: List[TableScala]) = new DataSetTableScala(tables.head.header, tables.flatMap(t => t.rows))
