...
class TaskLookup extends Task {
  ...
  def execute() = {
    ...
    val query = dataSourceConfig.getString("query")

    val inClause = dataSet.getColumnValues(lookupColumn1).distinct mkString ","

    val dataSource = DPSystemFactory.newDataSource(dataSourceConfig.withValue("query", ConfigValueFactory.fromAnyRef(query.replaceAllLiterally("$1", inClause))), masterConfig)
    dataSource.execute()
